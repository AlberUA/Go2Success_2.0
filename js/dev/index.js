import { g as getHash, d as dataMediaQueries, s as slideDown, a as setHash, b as slideUp, c as slideToggle, e as bodyLock, f as bodyUnlock, h as bodyLockStatus, i as bodyLockToggle, j as gotoBlock, u as uniqArray } from "./common.min.js";
(function polyfill() {
...
function initSliders() {
  ...
  if (document.querySelector(".tabs-about__slider")) {
    document.querySelectorAll(".tabs-about__slider").forEach((swiperEl) => {
      new Swiper(swiperEl, {
        // Підключаємо модулі слайдера
        // для конкретного випадку
        modules: [Navigation],
...
window.addEventListener("load", () => {
  ...
  const inputTelPopup = document.querySelector("#phonePopup");
  if (inputTel) {
    window.intlTelInput(inputTel, {
      ...
      nationalMode: false,
      // завжди показує код країни (+380)
      formatOnDisplay: true,
      // форматування при вводі
      strictMode: true,
      separateDialCode: true,
      // код країни не окремо
      loadUtils: () => __vitePreload(() => import("https://cdn.jsdelivr.net/npm/intl-tel-input@25.12.4/build/js/utils.js"), true ? [] : void 0, import.meta.url)
    });
  }
  if (inputTelPopup) {
    window.intlTelInput(inputTelPopup, {
      initialCountry: "ua",
      // Україна за замовчуванням
      autoInsertDialCode: "true",
      nationalMode: false,
...
window.addEventListener("load", () => {
  ...
  document.addEventListener("click", function(e) {
    const targetElement = e.target;
    const tabsAbout = document.querySelectorAll(".tabs-about__title");
    const tabsBodyAbout = document.querySelectorAll(".body-tabs-about");
    const tabsSelectOptionAbout = document.querySelectorAll(".select__option");
    if (targetElement.hasAttribute("data-for-kids") || targetElement.dataset.flsSelectValue == "forKids") {
      tabsAbout.forEach((tab) => {
        tab.classList.remove("--tab-active");
        document.querySelector(".tabs-about__title--1").classList.add("--tab-active");
        tabsBodyAbout.forEach((bodyEl) => {
          bodyEl.hidden = true;
        });
        document.querySelector(".body-tabs-about--kids").hidden = false;
        document.querySelector(".select__content").textContent = "Для дітей";
        tabsSelectOptionAbout.forEach((selectEl) => {
          selectEl.hidden = false;
        });
        document.querySelector('[data-fls-select-value="forKids"]').hidden = true;
      });
    } else if (targetElement.hasAttribute("data-for-teenagers") || targetElement.dataset.flsSelectValue == "forTeenagers") {
      tabsAbout.forEach((tab) => {
        tab.classList.remove("--tab-active");
        document.querySelector(".tabs-about__title--2").classList.add("--tab-active");
        tabsBodyAbout.forEach((bodyEl) => {
          bodyEl.hidden = true;
        });
        document.querySelector(".body-tabs-about--teenagers").hidden = false;
        document.querySelector(".select__content").textContent = "Для підлітків";
        tabsSelectOptionAbout.forEach((selectEl) => {
          selectEl.hidden = false;
        });
        document.querySelector('[data-fls-select-value="forTeenagers"]').hidden = true;
      });
    } else if (targetElement.hasAttribute("data-for-adults") || targetElement.dataset.flsSelectValue == "forAdults") {
      tabsAbout.forEach((tab) => {
        tab.classList.remove("--tab-active");
        document.querySelector(".tabs-about__title--3").classList.add("--tab-active");
        tabsBodyAbout.forEach((bodyEl) => {
          bodyEl.hidden = true;
        });
        document.querySelector(".body-tabs-about--adults").hidden = false;
        document.querySelector(".select__content").textContent = "Для дорослих";
        tabsSelectOptionAbout.forEach((selectEl) => {
          selectEl.hidden = false;
        });
        document.querySelector('[data-fls-select-value="forAdults"]').hidden = true;
      });
    } else if (targetElement.hasAttribute("data-for-professions") || targetElement.dataset.flsSelectValue == "forProfessions") {
      tabsAbout.forEach((tab) => {
        tab.classList.remove("--tab-active");
        document.querySelector(".tabs-about__title--4").classList.add("--tab-active");
        tabsBodyAbout.forEach((bodyEl) => {
          bodyEl.hidden = true;
          document.querySelector(".body-tabs-about--professions").hidden = false;
        });
        document.querySelector(".select__content").textContent = "Для професії";
        tabsSelectOptionAbout.forEach((selectEl) => {
          selectEl.hidden = false;
        });
        document.querySelector('[data-fls-select-value="forProfessions"]').hidden = true;
      });
    } else if (targetElement.dataset.flsSelectValue == "speakingClub") {
      tabsAbout.forEach((tab) => {
        tab.classList.remove("--tab-active");
        document.querySelector(".tabs-about__title--5").classList.add("--tab-active");
        tabsBodyAbout.forEach((bodyEl) => {
          bodyEl.hidden = true;
        });
        document.querySelector(".body-tabs-about--club").hidden = false;
      });
    }
  });
});
